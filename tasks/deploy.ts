...
task("deploy", "Deploy a ZKProofOfHumanity contract")
    ...
    .addParam("proofOfHumanity", "ProofOfHumanity contract address", undefined, types.string)
    .addOptionalParam("group", "Group id", "42", types.string)
    .addOptionalParam("logs", "Print the logs", true, types.boolean)
    .setAction(
        async ({ logs, semaphore: semaphoreAddress, proofOfHumanity: pohAddress, group: groupId }, { ethers, run }) => {
            if (!semaphoreAddress) {
